...
namespace OWSData.Repositories.Implementations.MySQL
{
    public class AccountRepository : IAccountRepository
    {
        ...
        public async Task<CreateCharacter> CreateCharacter(Guid customerGUID, Guid accountSessionGUID, string characterName, string className)
        {
            ...
            try
            {
                using (Connection)
                {
                    var p = new DynamicParameters();
                    p.Add("@CustomerGUID", customerGUID);
                    p.Add("@AccountSessionGUID", accountSessionGUID); // Corrected parameter name
                    p.Add("@CharacterName", characterName);
                    p.Add("@ClassName", className);

                    // Call the MySQL procedure AddCharacter
                    outputObject = await Connection.QuerySingleAsync<CreateCharacter>(
                        "call AddCharacter(@CustomerGUID, @AccountSessionGUID, @CharacterName, @ClassName)",
                        p,
                        commandType: CommandType.Text);
                }
        ...
        public async Task<SuccessAndErrorMessage> CreateCharacterUsingDefaultCharacterValues(Guid customerGUID, Guid accountID, string characterName, string defaultSetName)
        {
            ...
            try
            {
                ...
                parameters.Add("AccountID", accountID); // Changed UserGUID to AccountID for MySQL
                parameters.Add("CharacterName", characterName);
                parameters.Add("DefaultSetName", defaultSetName);

                // Execute the MySQL procedure to add character using default values
        ...
        public async Task<IEnumerable<GetPlayerGroupsCharacterIsIn>> GetPlayerGroupsCharacterIsIn(Guid customerGUID, Guid accountSessionGUID, string characterName, int playerGroupTypeID = 0)
        {
            ...
            using (Connection)
            {
                ...
                OutputObject = await Connection.QueryAsync<GetPlayerGroupsCharacterIsIn>("call GetPlayerGroupsCharacterIsIn(@CustomerGUID,@CharName,@AccountSessionGUID,@PlayerGroupTypeID)",
                    p,
                    commandType: CommandType.Text);
            }
        ...
        public async Task<Account> GetAccount(Guid customerGuid, Guid accountId)
        {
            ...
            using (Connection)
            {
                ...
                outputObject = await Connection.QuerySingleOrDefaultAsync<Account>(
                    "call GetAccount(@CustomerGUID, @AccountID)",
                    p,
                    commandType: CommandType.Text);
            }
        ...
        public async Task<GetAccountSession> GetAccountSession(Guid customerGUID, Guid accountSessionGUID)
        {
            ...
            using (Connection)
            {
                ...
                outputObject = await Connection.QuerySingleOrDefaultAsync<GetAccountSession>(
                    "select * from GetAccountSession(@CustomerGUID, @AccountSessionGUID)",
                    p,
                    commandType: CommandType.Text);
            }
        ...
        public async Task<GetAccountSession> GetAccountSessionORM(Guid customerGUID, Guid accountSessionGUID)
        {
            ...
            using (Connection)
            {
                // Execute the query using the provided parameters and map the result to the GetAccountSession class
                outputObject = await Connection.QueryFirstOrDefaultAsync<GetAccountSession>(
                    MySQLQueries.GetAccountSessionSQL,
        ...
        public async Task<GetAccountSessionComposite> GetAccountSessionParallel(Guid customerGUID, Guid accountSessionGUID) // id = AccountSessionGUID
        {
            ...
            using (Connection)
            {
                ...
                accountSession = await Connection.QueryFirstOrDefaultAsync<AccountSession>(MySQLQueries.GetAccountSessionOnlySQL, 
                    new { CustomerGUID = customerGUID, AccountSessionGUID = accountSessionGUID });

                // Get the Account and Character data in parallel
                var accountTask = Connection.QueryFirstOrDefaultAsync<Account>(MySQLQueries.GetAccountSQL, 
                    new { CustomerGUID = customerGUID, AccountID = accountSession.AccountID });

                var characterTask = Connection.QueryFirstOrDefaultAsync<Character>(MySQLQueries.GetCharacterByNameSQL, 
        ...
        public async Task<PlayerLoginAndCreateSession> LoginAndCreateSession(Guid customerGUID, string email, string password, bool dontCheckPassword = false)
        {
            ...
            using (Connection)
            {
                ...
                // Calling the PlayerLoginAndCreateSession function directly in MySQL
                outputObject = await Connection.QuerySingleOrDefaultAsync<PlayerLoginAndCreateSession>(
                    "select * from PlayerLoginAndCreateSession(@CustomerGUID, @Email, @Password, @DontCheckPassword)",
                    p,
                    commandType: CommandType.Text);
            }
        ...
        public async Task<GetAccountSession> GetAccountFromEmail(Guid customerGUID, string email)
        {
            ...
            {
                outputObject = await Connection.QueryFirstOrDefaultAsync<GetAccountSession>(
                    PostgresQueries.GetAccountFromEmailSQL, 
