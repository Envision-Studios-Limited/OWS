    ...
    public class AccountRepository : IAccountRepository
    {
        ...
        public async Task<SuccessAndErrorMessage> CreateCharacterUsingDefaultCharacterValues(Guid customerGUID, Guid accountID, string characterName, string defaultSetName)
        {
            ...
            try
            {
                ...
                parameters.Add("AccountID", accountID); // Using AccountID instead of UserGUID
                parameters.Add("CharacterName", characterName);
                parameters.Add("DefaultSetName", defaultSetName);

                // Execute the PostgreSQL function to add character using default values
        ...
        public async Task<GetAccountSession> GetAccountSession(Guid customerGUID, Guid accountSessionGUID)
        {
            ...
            using (Connection)
            {
                ...
                outputObject = await Connection.QuerySingleOrDefaultAsync<GetAccountSession>("select * from GetAccountSession(@CustomerGUID,@AccountSessionGUID)",
                    p,
                    commandType: CommandType.Text);
            }
        ...
        public async Task<GetAccountSession> GetAccountSessionORM(Guid customerGUID, Guid accountSessionGUID)
        {
            ...
            using (Connection)
            {
                ...
                outputObject = await Connection.QueryFirstOrDefaultAsync<GetAccountSession>(
                    PostgresQueries.GetAccountSessionSQL,
        ...
        public async Task<GetAccountSessionComposite> GetAccountSessionParallel(Guid customerGUID, Guid accountSessionGUID) //id = AccountSessionGUID
        {
            ...
            using (Connection)
            {
                ...
                accountSession = await Connection.QueryFirstOrDefaultAsync<AccountSession>(PostgresQueries.GetAccountSessionOnlySQL, 
                    new { CustomerGUID = customerGUID, AccountSessionGUID = accountSessionGUID });

                // Get the Account and Character data in parallel
                var accountTask = Connection.QueryFirstOrDefaultAsync<Account>(PostgresQueries.GetAccountSQL, 
                    new { CustomerGUID = customerGUID, AccountID = accountSession.AccountID });

                var characterTask = Connection.QueryFirstOrDefaultAsync<Character>(PostgresQueries.GetCharacterByNameSQL, 
        ...
        public async Task<PlayerLoginAndCreateSession> LoginAndCreateSession(Guid customerGUID, string email, string password, bool dontCheckPassword = false)
        {
            ...
            using (Connection)
            {
                ...
                // Calling the PlayerLoginAndCreateSession function directly
