...
namespace OWSData.Repositories.Implementations.MSSQL
{
    public class CharactersRepository : ICharactersRepository
    {
            ...
            using (Connection)
            {
                ...
                var results = await Connection.QueryAsync<GetItemDetails, string, string, ItemStatMapping, GetItemDetails>(
                    ...
                    (item, action, tag, stat) =>
                    {
                        ...
                        if (stat != null && !itemEntry.ItemStats.Any(s => s.ItemStatName == stat.ItemStatName && s.ItemStatValue == stat.ItemStatValue))
                        {
                    ...
                    p,
                    splitOn: "ItemAction,ItemTag,ItemStatValue" // Split the result into multiple objects
                );

                // Return the first (and only) item in the dictionary
                return itemDictionary.Values.FirstOrDefault();
            }
        }
        ...
        public async Task<TransferItemResult> TransferItemBetweenInventories(Guid customerGUID, int sourceCharacterInventoryID, int targetCharacterInventoryID,
            int itemQuantity, int sourceSlotIndex, int targetSlotIndex)
        {
